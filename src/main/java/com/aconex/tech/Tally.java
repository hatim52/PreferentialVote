...
/**
 * This class is used for methods used to tally votes for preferential voting.
 */
public class Tally {
     ...
     * @param votesList
     */
    public Tally (ArrayList<Vote> votesList){
    ...
    private void NormalizeMap(){
        if ( hm.size () < Candidate.getCandidateCount ()){
            //find out which candidates are not updated in hm
            Set<Character> hmSet = new HashSet<Character> (hm.keySet ());
            Set<Character> origSet = new HashSet<Character> (Candidate.candidates.keySet ());
